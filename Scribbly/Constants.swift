//
//  Constants.swift
//  Scribbly
//
//  Created by Vin Bui on 12/18/22.
//
import UIKit

/**
 Constants for Design
 */
struct Constants {
    // ------------ Other ------------
    static let reuse = "Reuse"
    
    
    // ------------ Home Page ------------
    static let border_top_padding = CGFloat(75)
    static let border_side_padding = CGFloat(15)
    static let prompt_heading_top_padding = CGFloat(10)
    static let prompt_top_padding = CGFloat(5)
    static let profile_button_radius = CGFloat(20)
    
    // ------------ PostCollectionView ------------
    static let post_cv_top_padding = CGFloat(15)
    static let post_cv_side_padding = CGFloat(15)
    static let post_container_width = UIScreen.main.bounds.width - 50
    static let post_container_height = CGFloat(450)
    
    // ------------ PostCollectionViewCell ------------
    static let post_cell_bg_color = UIColor.systemBackground
    static let post_cell_top_padding = CGFloat(10)
    static let post_cell_side_padding = CGFloat(10)
    static let post_cell_corner = CGFloat(15)
    static let post_cell_name_side = CGFloat(10)
    static let post_cell_caption_top = CGFloat(2)
    static let post_cell_drawing_top = CGFloat(15)
    static let post_cell_drawing_bot = CGFloat(50)
    static let post_cell_drawing_corner = CGFloat(10)
}

